...
import { compareRecipe } from "./recipeMatch.js?v=5";
...
async function loadData() {
  const tRes = await fetch("data/tests.json");
  if (!tRes.ok) throw new Error(`tests.json HTTP ${tRes.status}`);
  const tText = await tRes.text();
  try { ALL_TESTS = JSON.parse(tText); }
  catch (e) { throw new Error(`tests.json parse fail: ${e.message} | starts with: ${tText.slice(0, 80)}`); }

  const rRes = await fetch("data/recipes.json");
  if (!rRes.ok) throw new Error(`recipes.json HTTP ${rRes.status}`);
  const rText = await rRes.text();
  try { ALL_RECIPES = JSON.parse(rText); }
  catch (e) { throw new Error(`recipes.json parse fail: ${e.message} | starts with: ${rText.slice(0, 80)}`); }
}
