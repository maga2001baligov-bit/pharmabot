...
const screens = {};
["home", "modes", "search", "errors", "session", "result", "stats", "settings", "sessions"].forEach((n) => {
  ...
  if (q.type === "choice") {
    ...
    const rows = q.items.map((item, i) => `
      <div class="match-row row-ok">
        ...
        <div class="row-hint">Верно: <b>${q.answer[i] + 1}</b>. ${escapeHtml(q.categories[q.answer[i]])}</div>
      </div>`).join("");
