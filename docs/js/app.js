...
import { compareRecipe } from "./recipeMatch.js?v=2";
...
function startExamTimerIfNeeded() {
  ...
  examDeadline = Date.now() + EXAM_SECONDS[session.kind] * 1000;
  const tick = () => {
...
function renderStats() {
  ...
  document.getElementById("stats-body").innerHTML =
    periodsHtml + kindBlock("Тесты", st.tests) + kindBlock("Рецепты", st.recipes) + lastExamHtml + bestExamHtml;
  showScreen("stats");
}
