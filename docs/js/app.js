...
if (tg) {
  // Каждый вызов в своём try/catch: на разных версиях Telegram (или в обычном
  // браузере вне Telegram) отдельные методы могут кидать WebAppMethodUnsupported.
  // Раньше один неподдерживаемый метод обрывал всю остальную инициализацию —
  // это и было причиной белого экрана без темы в Safari.
  try { tg.ready(); } catch (e) { /* no-op */ }
  try { tg.expand(); } catch (e) { /* no-op */ }

  // Полноэкранный режим (Bot API 8.0+): без скруглённых углов и "шторки".
  // Поддержка проверяется дважды — и наличием метода, и try/catch на случай,
  // если метод объявлен в SDK, но не поддержан текущим клиентом/платформой.
  if (typeof tg.requestFullscreen === "function") {
    try { tg.requestFullscreen(); } catch (e) { /* остаёмся в обычном expand() */ }
  }
  // Отключаем случайное закрытие свайпом вниз по контенту (Bot API 7.7+).
  if (typeof tg.disableVerticalSwipes === "function") {
    try { tg.disableVerticalSwipes(); } catch (e) { /* no-op */ }
  }

  // В полноэкранном режиме приложение само отвечает за отступы под чёлку/
  // статус-бар и нижний индикатор.
  const applySafeArea = () => {
    try {
      const top = (tg.contentSafeAreaInset && tg.contentSafeAreaInset.top) || (tg.safeAreaInset && tg.safeAreaInset.top) || 0;
      const bottom = (tg.contentSafeAreaInset && tg.contentSafeAreaInset.bottom) || (tg.safeAreaInset && tg.safeAreaInset.bottom) || 0;
  ...
  if (typeof tg.onEvent === "function") {
    try {
      ...
      tg.onEvent("fullscreenFailed", () => { /* остаёмся в обычном expand()-режиме, это ок */ });
    } catch (e) { /* no-op */ }
  }
}
...
function initTheme() {
  ...
  if (!theme) {
    // first visit: "Ядро" — тёмная тема теперь основной дизайн приложения
...
// ---- нижняя навигация "Ядро": подсветка активной вкладки + позиция конуса света ----
function navKeyForScreen(name) {
  if (name === "home") return "home";
  if (name === "settings") return "settings";
  if (name === "stats" || name === "errors" || name === "achievements") return "stats";
  if (name === "modes" || name === "topics" || name === "session" || name === "search") {
    const kind = session ? session.kind : modesKind;
    return kind === "recipe" ? "recipes" : "tests";
  }
  return null;
}

function positionNavGlow(btn) {
  const nav = document.getElementById("nav-bar");
  const lamp = document.getElementById("nav-lamp");
  const cone = document.getElementById("nav-cone");
  if (!nav || !btn) return;
  const navRect = nav.getBoundingClientRect();
  const btnRect = btn.getBoundingClientRect();
  const centerInNav = btnRect.left - navRect.left + btnRect.width / 2;
  lamp.style.left = (centerInNav - 10) + "px";
  cone.style.left = (centerInNav - 44) + "px";
}
...
function updateNavActive(screenName) {
  ...
  const key = navKeyForScreen(screenName);
  const lamp = document.getElementById("nav-lamp");
  const cone = document.getElementById("nav-cone");
  if (!key) {
  ...
  const target = nav.querySelector(`[data-navkey="${key}"]`);
  if (!target) return;
  target.classList.add("active");
  lamp.style.opacity = "1";
  cone.style.opacity = "1";
  positionNavGlow(target);
}

window.addEventListener("resize", () => {
  const active = document.querySelector("#nav-bar .nav-btn.active");
  if (active) positionNavGlow(active);
});
...
/**
 * Maps the 11 broad test topics to the finer-grained recipe topic groups that
 * belong under them (the two taxonomies were built at different times, at
 * different granularity, so this bridges them). Used for the "🔗 related
 * topic" cross-links shown on every card.
 */
const TEST_TOPIC_TO_RECIPE_TOPICS = {
  "Гормональные средства": ["Гормональные средства", "Противодиабетические средства"],
  "Адренергические средства": ["Адреномиметики", "Адреноблокаторы"],
  "Холинергические средства": ["Холиномиметики и антихолинэстеразные средства", "Холиноблокаторы"],
  "Антибиотики и химиотерапия": [
    "Антибиотики: бета-лактамы", "Антибиотики: макролиды, аминогликозиды и др.",
    "Синтетические антимикробные средства", "Противотуберкулёзные средства", "Антисептики и дезинфицирующие средства",
  ],
  "Сердечно-сосудистые средства": [
    "Сердечные гликозиды и кардиотонические средства", "Антиаритмические средства", "Антиангинальные средства",
    "Блокаторы кальциевых каналов", "Гипотензивные средства", "Диуретики", "Гиполипидемические средства",
    "Средства, влияющие на свёртывание крови и гемостаз",
  ],
  "Противогрибковые и противопаразитарные": ["Противопаразитарные средства"],
  "ЖКТ и пищеварение": ["Средства, влияющие на ЖКТ"],
  "Аллергия, иммунитет, противовирусные": ["Противоаллергические (антигистаминные) средства", "Противовирусные средства"],
  "Дыхательная система": ["Бронхолитические и противоастматические средства"],
  "Витамины": ["Витамины и антианемические средства"],
  "Обмен веществ и почки": ["Диуретики", "Противодиабетические средства"],
};

/** Reverse of the map above: recipe topic -> parent test topic (built once). */
const RECIPE_TOPIC_TO_TEST_TOPIC = {};
for (const [testTopic, recipeTopics] of Object.entries(TEST_TOPIC_TO_RECIPE_TOPICS)) {
  recipeTopics.forEach((rt) => { RECIPE_TOPIC_TO_TEST_TOPIC[rt] = testTopic; });
}

/**
 * Small "🔗 related topic" link shown on every card, bridging tests and
 * recipes that cover the same pharmacological ground — the two content types
 * are otherwise completely separate worlds.
 */
function renderCrossLinkBox(area, kind, id) {
  const item = byId(kind, id);
  if (!item || !item.topic) return;
  const slip = area.querySelector(".slip") || area;

  let targetKind, targetTopics, label;
  if (kind === "test") {
    targetKind = "recipe";
    targetTopics = TEST_TOPIC_TO_RECIPE_TOPICS[item.topic] || [];
    label = "💊 Рецепты по этой теме";
  } else {
    targetKind = "test";
    const parent = RECIPE_TOPIC_TO_TEST_TOPIC[item.topic];
    targetTopics = parent ? [parent] : [];
    label = "📝 Тесты по этой теме";
  }
  if (!targetTopics.length) return;

  const targetItems = (targetKind === "test" ? ALL_TESTS : ALL_RECIPES).filter((x) => targetTopics.includes(x.topic));
  if (!targetItems.length) return;

  const btn = document.createElement("button");
  btn.className = "crosslink-btn";
  btn.innerHTML = `<span>${label}</span><span class="crosslink-n">${targetItems.length}</span>`;
  btn.addEventListener("click", () => startSession(targetKind, "topic", shuffle(targetItems.map((x) => x.id))));
...
function renderBadgeRow(today, st) {
  const el = document.getElementById("badge-row");
  if (!el) return;
  const { count: streak } = computeStreak();
  const doneToday = today.correct + today.wrong;
  const totalSeen = st.tests.seen + st.recipes.seen;
  const totalItems = st.tests.total + st.recipes.total;
  const overallPct = totalItems ? Math.round((totalSeen / totalItems) * 100) : 0;
  el.innerHTML = `
    <div class="badge"><div class="ic">🏅</div><div class="badge-n">${streak}</div><div class="badge-l">${streak === 1 ? "день подряд" : "дней подряд"}</div></div>
    <div class="badge"><div class="ic">⚡</div><div class="badge-n">${doneToday}</div><div class="badge-l">сегодня</div></div>
    <div class="badge"><div class="ic">🎯</div><div class="badge-n">${overallPct}%</div><div class="badge-l">материала пройдено</div></div>
  `;
}
...
document.querySelectorAll("[data-nav]").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    else if (nav === "tests") openModes("test");
...
function renderCurrent() {
  ...
  if (session.mode === "learn") {
    ...
    renderCrossLinkBox(area, session.kind, id);
    return;
  }
...
function shuffleMatchingQuestion(q) {
  if (!Storage.getSettings().shuffleAnswers) return q;
  const itemPerm = shuffle(q.items.map((_, i) => i));
  const catPerm = shuffle(q.categories.map((_, i) => i));
  const catOldToNew = {};
  catPerm.forEach((oldIdx, newIdx) => { catOldToNew[oldIdx] = newIdx; });
  const newItems = itemPerm.map((oldIdx) => q.items[oldIdx]);
  const newCategories = catPerm.map((oldIdx) => q.categories[oldIdx]);
  const newAnswer = itemPerm.map((oldIdx) => q.answer[oldIdx].map((c) => catOldToNew[c]));
  return { ...q, items: newItems, categories: newCategories, answer: newAnswer };
}

function renderMatchingCard(area, q) {
  q = shuffleMatchingQuestion(q);
...
function renderSearchResults() {
  ...
  if (searchKind === "all") {
    if (!term) {
      resultsEl.innerHTML = `<div class="search-empty">Начни печатать — ищу сразу в тестах, рецептах и теории</div>`;
      return;
    }
    const testHits = ALL_TESTS.filter((q) =>
      (numMatch && String(q.orig_num) === term) || questionPreviewText(q).toLowerCase().includes(term)
    ).slice(0, 20).map((q) => ({ kindTag: "Тест", cls: "test", label: questionPreviewText(q).slice(0, 90), id: q.id }));
    const recipeHits = ALL_RECIPES.filter((r) => recipePreviewText(r).toLowerCase().includes(term))
      .slice(0, 20).map((r) => ({ kindTag: "Рецепт", cls: "recipe", label: recipePreviewText(r).slice(0, 90), id: r.id }));
    const theoryHits = theorySearchResults(term).slice(0, 20)
      .map((t) => ({ kindTag: "Теория", cls: "theory", label: t.title, poolKey: t.poolKey, num: t.num }));
    const all = [...testHits, ...recipeHits, ...theoryHits];
    if (!all.length) {
      resultsEl.innerHTML = `<div class="search-empty">Ничего не найдено</div>`;
      return;
    }
    resultsEl.innerHTML = all.map((item, i) => `
      <button class="search-hit search-hit-tagged" data-i="${i}">
        <span class="search-tag search-tag-${item.cls}">${item.kindTag}</span>
        <span class="t">${escapeHtml(item.label)}</span>
      </button>
    `).join("");
    resultsEl.querySelectorAll(".search-hit-tagged").forEach((btn, i) => {
      btn.addEventListener("click", () => {
        const item = all[i];
        if (item.cls === "test") startSession("test", "single", [item.id]);
        else if (item.cls === "recipe") startSession("recipe", "single", [item.id]);
        else openTheoryAt(item.poolKey, item.num);
      });
    });
    return;
  }
...
function openSearch(kind) {
  ...
  document.getElementById("search-title").textContent =
    kind === "all" ? "Поиск по всему" : kind === "test" ? "Поиск по тестам" : "Поиск по рецептам";
  document.getElementById("filter-chips").classList.toggle("hidden", kind !== "test");
  document.getElementById("search-input").placeholder =
    kind === "all" ? "Тесты, рецепты, теория — сразу везде" :
...
if (globalSearchBtn) globalSearchBtn.addEventListener("click", () => openSearch("all"));
...
// ---------------------------------------------------------------------
// Шеринг прогресса как картинки (canvas -> PNG, скачать / native share)
// ---------------------------------------------------------------------
function drawProgressCard() {
  ...
  const xp = computeXP();
  const { cur } = computeRank(xp);
  const { count: streak } = computeStreak();
  const st = Storage.getStats(ALL_TESTS.length, ALL_RECIPES.length);

  // фон — тёмный + красное свечение сверху, как основной дизайн приложения
  ctx.fillStyle = "#0a0908";
  ctx.fillRect(0, 0, W, H);
  const glow = ctx.createRadialGradient(W * 0.5, -40, 20, W * 0.5, -40, 420);
  glow.addColorStop(0, "rgba(255,68,51,0.35)");
  glow.addColorStop(1, "rgba(255,68,51,0)");
  ctx.fillStyle = glow;
  ctx.fillRect(0, 0, W, H);
  ...
  ctx.fillText("Фармаколог", W / 2, 90);
  ctx.fillStyle = "#948d81";
  ctx.font = "500 18px Manrope, sans-serif";
  ctx.fillText("Подготовка к экзамену", W / 2, 122);

  // стрик — крупно, по центру
  ctx.fillStyle = "#ff4433";
  ctx.font = "800 130px Manrope, sans-serif";
  ctx.fillText(`🔥 ${streak}`, W / 2, 300);
  ctx.fillStyle = "#948d81";
  ctx.font = "600 20px Manrope, sans-serif";
  ctx.fillText(streak === 1 ? "день подряд" : "дней подряд", W / 2, 335);

  // ранг + xp
  ctx.fillStyle = "#f5f1ea";
  ctx.font = "700 24px Manrope, sans-serif";
  ctx.fillText(`${cur.icon} ${cur.name} · ${xp} XP`, W / 2, 400);

  // три плашки статистики
  const stats = [
    [`${st.tests.pct}%`, "точность в тестах"],
    [`${st.recipes.pct}%`, "точность в рецептах"],
    [`${st.tests.seen + st.recipes.seen}`, "вопросов пройдено"],
  ];
  const boxW = 180, boxH = 110, gap = 20;
  ...
  stats.forEach((s, i) => {
    ...
    ctx.fillText(s[0], x + boxW / 2, boxY + 48);
    ctx.fillStyle = "#948d81";
    ctx.font = "500 13px Manrope, sans-serif";
    wrapCenteredText(ctx, s[1], x + boxW / 2, boxY + 72, boxW - 20, 16);
  });

  ctx.fillStyle = "#5a544c";
  ctx.font = "500 14px 'IBM Plex Mono', monospace";
  ctx.fillText("PharmaBot · @farma_trainer_bot", W / 2, H - 30);
}
...
async function openShareModal() {
  ...
  document.getElementById("share-download-link").href = dataUrl;

  const nativeBtn = document.getElementById("share-native-btn");
  if (navigator.share && navigator.canShare) {
    canvas.toBlob((blob) => {
      const file = new File([blob], "pharmabot-progress.png", { type: "image/png" });
      if (navigator.canShare({ files: [file] })) {
        nativeBtn.classList.remove("hidden");
        nativeBtn.onclick = () => navigator.share({
          files: [file],
          title: "Мой прогресс в PharmaBot",
        }).catch(() => {});
      }
    });
  }
...
const shareBtn = document.getElementById("btn-share-progress");
