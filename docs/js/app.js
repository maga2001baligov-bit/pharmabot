...
import { compareRecipe } from "./recipeMatch.js?v=15";
...
if (tg) {
  ...
  // как у полноразмерных мини-приложений. Проверяем поддержку — на старых
  // клиентах Telegram метода может не быть, тогда просто остаёмся в expand().
  if (typeof tg.requestFullscreen === "function") {
    tg.requestFullscreen();
  }
  // Отключаем случайное закрытие свайпом вниз по контенту (Bot API 7.7+).
  // Свайп по самой шапке Telegram всё равно закроет приложение — это
  // ограничение самого Telegram, не обходится и на это не стоит рассчитывать.
  if (typeof tg.disableVerticalSwipes === "function") {
    tg.disableVerticalSwipes();
  }

  // В полноэкранном режиме приложение само отвечает за отступы под чёлку/
  // статус-бар и нижний индикатор (раньше это брал на себя Telegram).
  const applySafeArea = () => {
