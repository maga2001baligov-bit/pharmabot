...
const MODE_LABELS = {
  ...
  topic: "По группе", confusables: "Путающиеся препараты",
};
...
function startSession(kind, mode, presetQueue) {
  ...
  session = { id: Date.now(), kind, mode, queue, pos: 0, correct: 0, wrong: 0, wrongIds: [], startedAt: new Date().toISOString() };
...
function grade(kind, id, ok) {
  ...
  if (ok) session.correct++; else { session.wrong++; session.wrongIds.push(id); }
  haptic(ok ? "ok" : "bad");
  persistSession();
