...
import { compareRecipe } from "./recipeMatch.js?v=10";
...
const screens = {};
["home", "modes", "search", "errors", "session", "result", "stats", "settings", "sessions", "achievements", "unlock", "theory-hub", "theory-list"].forEach((n) => {
...
function computeXP() {
  ...
  return sumEntries(t, "correct") * 10 + sumEntries(r, "correct") * 15;
}
...
function renderTheoryList(poolKey) {
  const pools = poolKey === "all" ? THEORY_POOLS : THEORY_POOLS.filter((p) => p.key === poolKey);
  const title = poolKey === "all" ? "Все материалы" : pools[0].label;
  document.getElementById("theory-list-title").textContent = title;

  const body = document.getElementById("theory-list-body");
  body.innerHTML = pools.map((p) => {
    const items = (ALL_THEORY[p.key] || []).map((q) => `
      <div class="acc-item">
        <div class="acc-head">
          <span class="acc-title">${q.num}. ${escapeHtml(q.title)}</span>
          <span class="acc-badge">открыть</span>
        </div>
        <div class="acc-body"><div class="theory-text">${renderFlashcardBody(q.body, true)}</div></div>
      </div>
    `).join("");
    return poolKey === "all"
      ? `<div class="section-head"><span class="section-title">${p.label}</span></div>${items}`
      : items;
  }).join("");
  ...
  body.querySelectorAll(".acc-item").forEach((item) => {
    item.querySelector(".acc-head").addEventListener("click", () => {
      ...
      body.querySelectorAll(".acc-item.open").forEach((el) => { el.classList.remove("open"); el.querySelector(".acc-badge").textContent = "открыть"; });
      if (!wasOpen) {
        item.classList.add("open");
        item.querySelector(".acc-badge").textContent = "свернуть";
...
document.querySelectorAll("[data-back]").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    if (dest === "home") renderHome();
      ...
      openModes(fromSearch ? searchKind : (session ? session.kind : "test"));
    }
  });
});
...
function startSession(kind, mode, presetQueue) {
  ...
  session = { id: Date.now(), kind, mode, queue, pos: 0, correct: 0, wrong: 0, startedAt: new Date().toISOString() };
