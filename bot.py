...
def _find_data_dir() -> str:
    ...
    (root/data, root/bot/data, root/docs/data). Pick whichever one actually
    contains tests.json, instead of assuming a single fixed path."""
    candidates = [
        os.path.join(BASE_DIR, "data"),
        os.path.join(BASE_DIR, "bot", "data"),
        os.path.join(BASE_DIR, "docs", "data"),
    ]
    for c in candidates:
        if os.path.isfile(os.path.join(c, "tests.json")):
            return c
    tried = "\n".join(f"  - {c}" for c in candidates)
    raise RuntimeError(
        f"tests.json не найден ни в одной из ожидаемых папок:\n{tried}\n"
        "Проверь, что tests.json и recipes.json действительно загружены в репозиторий."
    )
